...
{
    int32_t sessionId = DistributedInputTransportBase::GetInstance().GetSessionIdByDeviceId(deviceId);
    if (sessionId < 0) {
...
{
    int32_t sinkSessionId = DistributedInputTransportBase::GetInstance().GetSessionIdByDeviceId(peerDevId);
    if (sinkSessionId < 0) {
...
{
    int32_t sinkSessionId = DistributedInputTransportBase::GetInstance().GetSessionIdByDeviceId(deviceId);
    if (sinkSessionId < 0) {
...
{
    int32_t sessionId = DistributedInputTransportBase::GetInstance().GetSessionIdByDeviceId(srcId);
    if (sessionId < 0) {
    ...
    const nlohmann::json &recMsg)
{
    ...
    std::string deviceId = DistributedInputTransportBase::GetInstance().FindDeviceBySession(sessionId);
    if (deviceId.empty()) {
    ...
    const nlohmann::json &recMsg)
{
    ...
    std::string sinkDevId = DistributedInputTransportBase::GetInstance().FindDeviceBySession(sessionId);
    if (sinkDevId.empty()) {
